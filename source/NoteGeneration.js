class NoteGeneration {
  constructor(){
    ...
    this.intervals = ["perfect unison", "minor second", "major second", "minor third", "major third", "perfect fourth", "diminished fifth", "perfect fifth", "minor sixth", 
                      "major sixth", "minor seventh", "major seventh", "octave"];
      
  }
  
  randomInterval(){ //interval always higher
    let root = this.getRandomNumber(0, 48); //get random note
    let interval = this.getRandomNumber(0,12); //get random interval, 13 possibilities from unison to octave
    
    
    return [this.dictionary[root], 
            this.dictionary[root + interval], //gets the higher note from array
